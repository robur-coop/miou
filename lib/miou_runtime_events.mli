(** {1 The Miou [Trace] system with [Runtime_events].}
    ...
    function. To ensure that the user has a trace of Miou events, they must
    call [Miou.Trace.set_reporter Miou_runtime_events.reporter]. It should be
    noted that the trace is only emitted if the environment variable
    [MIOU_TRACE=1] exists.
