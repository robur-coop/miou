...
module Trigger = struct
  ...
  let await trigger = match await trigger with
...
module Computation = struct
  ...
  let cancelled t = match canceled t with
    | Some { Picos_exn_bt.exn; bt; } -> Some (exn, bt)
    | None as none -> none
  ...
  let peek t = match peek t with
    | None as none -> none
    | Some (Ok _) as some -> some
    | Some (Error { Picos_exn_bt.exn; bt; }) ->
      Some (Error (exn, bt))

  let await t = match await t with
    | value -> Ok value
    | exception exn ->
