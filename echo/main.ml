...
let listen = function
  | Unix.ADDR_INET (inet_addr, _) as sockaddr ->
      if Unix.is_inet6_addr inet_addr then (
        let tcpv6 = Miou_unix.tcpv6 () in
        Miou_unix.bind_and_listen tcpv6 sockaddr;
        tcpv6)
      else
        let tcpv4 = Miou_unix.tcpv4 () in
        Miou_unix.bind_and_listen tcpv4 sockaddr;
        tcpv4
  | _ -> failwith "Impossible to listen into a unix file"

let handler fd () =
  let rec go buf =
    match Miou_unix.read fd buf 0 (Bytes.length buf) with
    | 0 -> Miou_unix.close fd
    | len ->
        let str = Bytes.unsafe_to_string buf in
        Miou_unix.write fd str 0 len;
        go buf
  in
...
let server sockaddr =
  let rec go orphans fd =
    ...
    let _ = Miou.call ~orphans (handler fd') in
    go orphans fd
  in
